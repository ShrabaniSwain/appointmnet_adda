...
//
                    ...
                    Glide.with(requireContext())
                        ...
                        .apply(RequestOptions.placeholderOf(R.drawable.baseline_person_24))
                        .into(headerBinding.imageView)
                    headerBinding.tvPhone.text = Editable.Factory.getInstance().newEditable(response.body()?.data?.app_user_data?.user_mobile
                        ?: ""
                    )
                    ...
                    headerBinding.tvEmail.text = Editable.Factory.getInstance().newEditable(response.body()?.data?.app_user_data?.user_email
                        ?: ""
                    )
                    headerBinding.tvName.text = Editable.Factory.getInstance().newEditable(response.body()?.data?.app_user_data?.user_profile_name
                        ?: ""
                    )
    ...
    private fun showLogoutConfirmationDialog() {
        val dialogView = LayoutInflater.from(requireContext()).inflate(R.layout.logout_dialog_box, null)
    ...
    private fun getServiceProviderDashboard() {
        ...
        val call = RetrofitClient.api.getServiceProviderDashboard(
            ...
            Constant.APP_USER_KEY,
            Constant.APP_USER_ID.toString(),
            Constant.APP_VERSION_NAME.toString(), "0", "0", "undefined","undefined","undefined","736101","undefined"
        )
        call.enqueue(object : Callback<DashboardProviderResponse> {
            override fun onResponse(call: Call<DashboardProviderResponse>, response: Response<DashboardProviderResponse>) {
                if (response.isSuccessful) {
                    hideProgressBar()
                    ...
                    binding.tvActiveAppointmentCount.text = response.body()?.data?.active_appointment.toString()
                    binding.tvCompletedAppointments.text = response.body()?.data?.completed_appointments.toString()
                    binding.tvCountCustomer.text = response.body()?.data?.total_customer.toString()
                    binding.tvCountBusiness.text = response.body()?.data?.total_business.toString()
                    binding.tvReceivedPayments.text = response.body()?.data?.received_payments.toString()

                    appointment = response.body()?.data?.appoinsments ?: emptyList()
                    enquiry = response.body()?.data?.enquiry ?: emptyList()

                    providerAppointmnetAdapter = ProviderAppointmnetAdapter(requireContext(), appointment)
                    binding.rvAppointment.layoutManager = LinearLayoutManager(requireContext(), LinearLayoutManager.HORIZONTAL, false)
                    binding.rvAppointment.adapter = providerAppointmnetAdapter

                    providerEnquiryItemAdapter = ProviderEnquiryItemAdapter(requireContext(), enquiry)
                    binding.rvEnquiry.layoutManager = LinearLayoutManager(requireContext(), LinearLayoutManager.HORIZONTAL, false)
